  ...
  namespace lp
  {
        ...
        const net::GraphStorage *graph)
    {
      ...
      for (uint32_t i = 0; i < num_demands; ++i)
      {
        ...
        CHECK(safe_strtod(line_split[3], &demand_bps));
      ...
      for (const auto &src_and_dst_and_demand : total_demands)
      {
        ...
        net::Bandwidth demand = net::Bandwidth::FromBitsPerSecond(demand_bps);
